...
# Classification Tree
_NOTE: I have used "The Elements of Statistical Learning" extensively and the `tree` function to build decision trees, as opposed to the `train` function in `caret`. I believe this provides greater control and understanding._
...
The resulting classification tree using default options.
```{r}
plot(bigTree)
```
...
Step 3 is to prune back the tree. I will choose to prune back at 22 nodes.
```{r}
# (3) DO THE PRUNING:
...
Now that we have a model, we need to find the prediction accuracy on unseen data (on the validation set). We note that `treePruned` has 0.7388 accuracy while `treeDefault` has 0.6472 accuracy.
```{r}
...
# Random Forest
_NOTE: I have used "The Elements of Statistical Learning" extensively and the `randomForest` function to build random forests, as opposed to the `train` function in `caret`. I believe this provides greater control and understanding._

Random forests, like bagging, use a number of decision trees on bootstrapped training samples. When building these trees for a random forest a _random sample_ of $m$ predictors is chosen as split candidates from the full set of $p$ predictors. The effect of this is to _decorrelate_ the trees. This builds on the notation that averaging predictors result in better predictions.
...
With random forests, we gain prediction accuracy at the cost of interpretability. Here we do not have a single decision tree that we can plot. We can look at variable importance plots, however, should we wish to see which variables have a large influence on the response. We can also look at the cross-validation error as a function of the number of trees in our model. We note that 100 (even 50!) trees are sufficient in the model: we see little improvement in OOB error beyond this point.
...
I will, however, keep 200 trees since this ran relitively quickly and there is no need to go back and build a smaller forest.
...
We see that prediction accuracy is sufficient to ace the quiz! Let's take the quiz:
...
Indeed, we get 100% on the quiz using the random forest and there is no need to consider a time-consuming boosted model or stacking! :)